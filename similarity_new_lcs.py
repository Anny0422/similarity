...
class CacuSimil(object):
    ...
    def get_idcorpus(self, saved_files):
        ...
        return contents, corpus, main_id, counts
    ...
    def get_similar(self, craw_file, dictionary, tfidf_vectors):
         #print(craw_file)
         content = craw_file['content'].replace('\n', '')
         print(content)
    ...
    # def get_lcs_score_list(self, contents, craw_file):
    #     lcs = LCS()
    #     content = craw_file['content']
    #     lcs_score_list = []
    #     for i in range(len(contents)):
    #         score = lcs.LCS_score(contents[i], content)
    ...
    def insert_or_counts(self, result, craw_file, contents, main_id, counts): # main_id, counts
         ...
         for i in range(len(result)):
             lcs.counter = 0
             tfidf_score_list.append(result[i][1])   #获得tfidf方法得到的与每个新闻的相似度

             content_i = contents[i]
             craw_content = craw_file['content'].replace('\n', '')
         ...
         if max(tfidf_score_list) <0.9 and max(lcs_score_list) < 0.86:
             ...
             print('与lcs分数最接近的数据id为', main_id[max_lcs_index], 'lcs分值为：', max(lcs_score_list))

             self.insert = 'INSERT INTO finance_news(content,date, id, tags, time,' \
                      ' update_time, url, website) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)'
             db.execute(self.insert, [craw_file['content'].replace('\n', ''), craw_file['date'], craw_file['id'], \
...
if __name__ == '__main__':
    ...
    if craw_file:
        ...
        contents, corpus, main_id, counts = CS.get_idcorpus(saved_files)
        dictionary, tfidf_vectors = CS.gen_Model(corpus)
        result = CS.get_similar(craw_file, dictionary, tfidf_vectors)
        #print(result)
        CS.insert_or_counts(result, craw_file, contents, main_id, counts)
